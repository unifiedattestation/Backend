...
export default function OemPage() {
  ...
  const [importResult, setImportResult] = useState<{
    ...
    buildPolicy: { id: string; buildFingerprint: string; created: boolean };
    anchor: { id: string; rsaSerialHex: string; ecdsaSerialHex: string } | null;
    matchedAuthorityName?: string;
    warnings: string[];
  } | null>(null);
  ...
  const importDevice = async () => {
    ...
    try {
      ...
      setImportError("Invalid JSON — paste the full JSON from the Android Service.");
      setImportLoading(false);
      return;
    }
    const res = await fetch(`${backendUrl}/api/v1/oem/import-device`, {
      method: "POST",
      headers: {
  ...
  return (
    <Layout>
      ...
      <section className="mt-8 bg-white/70 rounded-2xl p-6 shadow-sm">
        <h2 className="text-xl font-semibold">Import from Device JSON</h2>
        <p className="text-xs text-gray-500 mt-1">
          Paste the JSON generated by the Android Service "Submit Device" button. This will create
          the device family, build policy, and trust anchor in one step.
        </p>
        <div className="mt-4 space-y-3">
          <textarea
            className="w-full rounded-lg border border-gray-300 px-3 py-2 font-mono text-xs"
            rows={8}
            placeholder='{ "device": { "codename": "..." }, "buildPolicy": { ... }, "trustAnchor": { ... }, ... }'
            value={importJson}
          ...
          {importResult && (
            <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 space-y-1 text-sm">
              <div className="font-semibold text-green-800">Import successful</div>
              <div className="text-gray-700">
                Device family:{" "}
                <span className="font-mono">{importResult.deviceFamily.codename}</span>
                {importResult.deviceFamily.created ? " (created)" : " (already existed)"}
              </div>
              <div className="text-gray-700">
                Build policy:{" "}
                <span className="font-mono text-xs">{importResult.buildPolicy.buildFingerprint}</span>
                {importResult.buildPolicy.created ? " (created)" : " (already existed)"}
              </div>
              {importResult.matchedAuthorityName && (
                <div className="text-gray-500 text-xs">
                  Matched authority: <span className="font-mono">{importResult.matchedAuthorityName}</span>
                </div>
              )}
              {importResult.anchor ? (
                <div className="text-gray-700">
                  Anchor registered — EC: <span className="font-mono text-xs">{importResult.anchor.ecdsaSerialHex}</span>
                </div>
              ) : (
                <div className="text-gray-500 text-xs">No anchor registered.</div>
              )}
